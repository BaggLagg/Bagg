...
GetChatLine(Line, timestamp=0, color=0){
    ...
    file := ""
    if(!timestamp)
return RegExReplace(output, "U)^\[\d{2}:\d{2}:\d{2}\] ")
    if(!color)
return RegExReplace(output, "Ui)\{[a-f0-9]{6}\}")
    return
}
getPlayerHealth() {
    if(!checkHandles())
...
showDialog(1, "{FFC800}Выбор игрока", "Введите ID игрока:","Запомнить","Закрыть")
Input, SDI, v, {Enter}
    if(isDialogOpen())
    {
    ...
    Return
                       ...
                       Numpad1::
                    {
                    SendChat("/me достал из кармана КПК и разблокировал экран" )
                    sleep 2000
                    SendChat("/me зашел в раздел База данных МВД")
                    Sleep 2000
                    SendChat("/me отправил запрос на проверку данных")
                    Sleep 2000
                    SendChat("/do Результаты по запросу получены.")
                    Sleep 2000
                    SendChat("/do На экране КПК высветился фоторобот преступника.")
                    sleep 2000
                    SendChat("/me выключил КПК и убрал его обратно в карман")
                    Sleep 2000
                    SendChat("Здравствуйте, я сотрудник федеральной правоохранительной структуры " nickname ".")
                    sleep 2000
                    SendChat("Следуйте за мной, в противном случае я применю силу.")
                    }   
                    Return

   Numpad2::
   {
   Sendchat("/do Наручники висят на поясе.")
   sleep 2000
   Sendchat("/me резким движением руки снял наручники с пояса")
   sleep 250
   SendChat("/cuff " SDI)
   }
   Return
   ...
   Numpad3::
   {
   Sendchat("/do Преступник крепкого теложложения.")
   sleep 2000
   Sendchat("/me резким движением заломал руку " name)
   sleep 250
   SendChat("/hold " SDI)
   sleep 2000
   }
...
Numpad4::
{
Sendchat("/me открыл дверь")
Sleep 2000
Sendchat("/me придерживая преступника, посадил его в машину")
Sleep 250
SendChat("/cuff " SDI)
Sleep 2000
...
Numpad5::
{
Sendchat("/me протянув руку к дверной ручке дернул за нее")
Sleep 2000
Sendchat("/do Дверь открылась.")
Sleep 2000
Sendchat("/me схватил преступника за руку и вытщил его из машины")
Sleep 2000
Sendchat("/me закрыл дверь")
Sleep 2000
}
...
Numpad6::
{
SendChat("/me достал КПК из внутреннего кармана")
Sleep 2000
SendChat("/me ввел информацию о преступнике")
Sleep 2000
SendChat("/me отправил информацию в полицейский департамент")
Sleep 2000
SendChat("/me передал преступника в полицейский департамент")
Sleep 2000
SendChat("/arrest " SDI)
Sleep 2000
}
       {
        showDialog(1, "{FFC800}Доклад", "Введите ТЭГ вашего подразделение`n Пример: DEA, NS, PS, CID:","Записать","Закрыть")
        Input, vizov, v, {Enter}
            if(vizov == "PS" or vizov == "DEA" or vizov == "NS" or vizov == "CID")
            {
            addChatMessage("{FFFFFF}• {FFC800}[Подсказка]{ffffff} Вы выбрали ТЕГ: " vizov)
           SendChat("/me сказал что-то в рацию...")
           Sleep 2000
         ShowDialog(1, "{FFC800}Доклад", "Введите ник вашего напарника(-ов)`n Пример: Tim Cookie, Cookie, Tim","Записать","Закрыть")
         Input, naparnik, v, {Enter}
         if(isDialogOpen())

 
                SendChat("/b " vizov  " | Закрыл дело: " name ".Напарник: " naparnik " ")
            }
            else
            {
            addChatMessage("{ffffff}• {FF0000}[Ошибка] {ffffff}Неккоректно введены данные, повторите попытку")
        ...
        Return
                      ...
                      {
                      sleep 250
                      ShowDialog(0,"{FFFFFF}[ Помощь по скрипту ]","{666699}- {FFFFFF}[ {666699}1{FFFFFF} ]{01FCFF} - Alt + 1{FFFFFF} - выбор игрока.`n{666699}- {FFFFFF}[ {666699}2{FFFFFF} ]{01FCFF} - NumPad 1{FFFFFF} - сверить фоторобот.`n{666699}- {FFFFFF}[ {666699}3{FFFFFF} ]{01FCFF} - NumPad 2{FFFFFF} - одеть наручники.`n{666699}- {FFFFFF}[ {666699}4{FFFFFF} ]{01FCFF} - NumPad 3{FFFFFF} - вести за собой.`n{666699}- {FFFFFF}[ {666699}5{FFFFFF} ]{01FCFF} - NumPad 4{FFFFFF} - Посадить в машину.`n{666699}- {FFFFFF}[ {666699}6{FFFFFF} ]{01FCFF} - Numpad 5{FFFFFF} - вытащить из машины.`n{666699}- {FFFFFF}[ {666699}7{FFFFFF} ]{01FCFF} - Numpad 6{FFFFFF} - Арестовать(Авто доклад).`n{666699}- {FFFFFF}[ {666699}8{FFFFFF} ]{01FCFF} - /helpAHK{FFFFFF} - помощь по скрипту.`n{666699}- {FFFFFF}[ {666699}9{FFFFFF} ]{01FCFF} - /miranda{FFFFFF} - зачитать миранду.`n{006666}          ver. 1.1 by Tim Cookie", "Закрыть")
                      }
 Return
...
:?:/miranda::
        {
            ...
            Sleep 2000
            SendChat("Если вы не можете оплатить услуги адвоката...")
            Sleep 2000
